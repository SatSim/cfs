...
;	Table that contains a cFE Non-Core Application that triggers a Software
;	Bus (SB) message to be sent when the application fails to check-in
;
;  Requirements Tested
;       None
;
...
$SC_$CPU_HS_AMT[4].ActionType = HS_AMT_ACT_NOACT
...
$SC_$CPU_HS_AMT[6].ActionType = HS_AMT_ACT_LAST_NONMSG+2

;; Make the rest of the entries empty
for index = 7 to HS_MAX_CRITICAL_APPS do
...
s create_tbl_file_from_cvt ("$CPU",apid,"App Monitoring Table Load 8","hs_def_amt8",amtTblName,"$SC_$CPU_HS_AMT[1].AppName",endmnemonic)

write ";*********************************************************************"
write ";  End procedure $SC_$CPU_hs_amt8 "
write ";*********************************************************************"
ENDPROC
