...
;   functions that have a file name as an argument. The arguments are setup to
;   contain special characters and this procedure documents which characters are
...
;                               removed all but the End test in order to
...
write ";*********************************************************************"
write ";  Step 2.2: Special Characters as last character in file extension. "
write ";*********************************************************************"
;; Last test for DirListFile - File name only
Invalid_DirLstFile2 = """"
OSError_DirLstFile2 = """"
Allowed_DirLstFile2 = """"
...
local testDirName
local testFileName, testFileNameOnly
local validSpec
...
FOR testNameNum = 1 to totaltests DO
  ;; Setup the dir and file names for this iteration
  testDirName = fileSystem & forwardSlash & %name("testchar" & testNameNum)
  testFileName = testDestDir & forwardSlash & beginFileName & endFileName & extension & %name("testchar" & testNameNum)
  testFileNameOnly = beginFileName & endFileName & extension & %name("testchar" & testNameNum)

  ;; Dir Create Command
  ;; This is being done in order to verify if the special character is valid
  errcnt = $SC_$CPU_FM_CMDEC + 1
  if (errcnt = 256) then
    errcnt = 0
  endif
  ...
  if (validSpec = "No") then
    ...
    s $sc_$cpu_fm_dirfiledisplay (testDestDir,testFileName,downloadDir,testFileNameOnly)
  endif
  ...
  /$sc_$cpu_FM_FileCat File1=testFileName File2=testFile DestName=concatOutFile1
  ...
  /$sc_$cpu_FM_FileCat File1=testFile File2=testFileName DestName=concatOutFile2
...
write ""
write ";*********************************************************************"
write ";  End procedure $sc_$cpu_fm_specialchars5                            "
write ";*********************************************************************"
ENDPROC
