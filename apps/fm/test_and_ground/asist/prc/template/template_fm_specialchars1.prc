...
;				removed all but the Alone test in order to
;				make the test run shorter.
;       03/01/11   W. Moleski   Added variables for App name and ram directory
;       08/18/11   W. Moleski   Added Overwrite argument to Copy and Move cmds
...
write ";*********************************************************************"
write ";  Step 2.2: Special Characters Alone test "
write ";*********************************************************************"
; LOG THE SPECIAL CHARACTER ATTEMPTS AND RESULTS
Invalid_Create = """"
OSError_Create = """"
Allowed_Create = """"
...
FOR testNameNum = 1 to totaltests DO
  ...
  testDirName = fileSystem & forwardSlash & %name("testchar" & testNameNum)
  testFileName = testDestDir & forwardSlash & %name("testchar" & testNameNum)
  testDownloadDir = fsDownloadDir & forwardSlash & %name("testchar" & testNameNum)
  testFileName2 = testDirName & forwardSlash & %name("testchar" & testNameNum)
  ...
  if (errcnt = $SC_$CPU_FM_CMDEC) then
    ...
    if (cmdctr = $SC_$CPU_FM_CMDPC) then
      ...
      if ($SC_$CPU_find_event[1].num_found_messages = 1) THEN
        ...
        write "<!> Failed (1003) - Event message ", FM_CREATE_DIR_CMD_EID, " NOT received.  (Cmd received)"
        ut_setrequirements FM_1003, "F"
      endif
    endif
  endif
  ...
  if (validSpec = "No") then
    s $sc_$cpu_fm_dirfiledisplay (testDestDir,testFileName,downloadDir,%name("testchar" & testNameNum),"Fail")
  else
    s $sc_$cpu_fm_dirfiledisplay (testDestDir,testFileName2,testDownloadDir,%name("testchar" & testNameNum))
  endif
  ...
  if (errcnt = $SC_$CPU_FM_CMDEC) then
    ...
    if (cmdctr = $SC_$CPU_FM_CMDPC) then
      ...
      if ($SC_$CPU_find_event[1].num_found_messages = 1) THEN
        write "<*> Passed (1003;3002.1) - Event message ",$SC_$CPU_find_event[1].eventid, " received (Cmd received)"
        ut_setrequirements FM_1003, "P"
        ut_setrequirements FM_3002_1, "P"
      else
        write "<!> Failed (1003;3002.1) - Event message ",FM_GET_DIR_FILE_CMD_EID, " NOT received.  (Cmd received)"
        ut_setrequirements FM_1003, "F"
        ut_setrequirements FM_3002_1, "F"
      endif
    endif
  ...
  if (errcnt = $SC_$CPU_FM_CMDEC) then
    ...
    if (cmdctr = $SC_$CPU_FM_CMDPC) then
      ...
      if ($SC_$CPU_find_event[1].num_found_messages = 1) THEN
        ...
        write "<!> Failed (1003) - Event message ", FM_DELETE_DIR_CMD_EID, " NOT received.  (Cmd received)"
        ut_setrequirements FM_1003, "F"
      endif
    endif
  endif
...
write ""
write ";*********************************************************************"
write ";  End procedure $sc_$cpu_fm_specialchars1                            "
write ";*********************************************************************"
ENDPROC
